...
phy_gen_out   <- str_glue("{data}/pseq_genus.Rds")
bray_dist_out <- str_glue("{data}/bray_dist.Rds")
bray_ord_out  <- str_glue("{data}/bray_ord.Rds")
df_final_out  <- str_glue("{data}/df_final.Rds")


# Outcomes
outcomes_nicenames <- c(BL1ST_J10_PNEUMONIA = "Pneumonia", BL1ST_INPAT_INFECT_HELIUS_LOWER_RESPIRATORY_TRACT = "Pneumonia_helius")
...
P.all <- readRDS(path.list$phy_file)
...
#randsamples <- sample(colnames(otu_table(P.all)), 200)
...
#-------------------------------------------------------------------------------------------------
# Agglomerate from the highest resolution level

print("Agglomerating to Genus level...")
P.genus <- tax_glom(P.all, "Genus")
print("P.genus, after agglomeration:")
P.genus

# Subset
coms <- intersect(sample_names(P.genus), sample_names(P))
P.genus <- prune_samples(coms, P.genus)
# .. and pick the metadata from the processed final object
sample_data(P.genus) <- sample_data(P)[sample_names(P.genus), ]
# Filter out rare taxa
P.genus <- core(P.genus, detection=1, prevalence=10/100, include.lowest=T)
...
#AK: get.otu.melt is function from problematic 'yingtools2' packages. 
#However all depences of this function are present, so sourced from 'functions.R' file 
...
print("P.CLR, butyrate producers")
P.CLR
...
#Beta diversity
saveRDS(bray, bray_dist_out) 
saveRDS(ord, bray_ord_out) 
